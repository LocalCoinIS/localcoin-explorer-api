...
def get_gate_data():
	url = "https://llcgateway.localcoin.is/?methodnameaction=GetAllowCurrency"
...
def get_assets_cmc():
	...
	for asset in assets:
		...
		canDeposit  = "False"
                minWithdrawal = 0
                maxWithdrawal = sys.maxsize


		if gate_data is not None and 'deposit' in gate_data and 'withdraw' in gate_data:
                	for item in gate_data['deposit']:
				if item['asset'].upper() == asset["symbol"].upper():
					canDeposit  = "True"
			for item in gate_data['withdraw']:
                                if item['asset'].upper() == asset["symbol"].upper():
                                        canWithdraw   = "True"
					minWithdrawal = item['minimalAmountWithdraw']

		items.append({
			"lastUpdateTimestamp": datetime.datetime.now().strftime('%Y-%m-%dT%H:%M:%S.000Z'),
			"name": asset["symbol"],
			"Unified Cryptoasset ID": asset["id"],
			"canWithdraw": canWithdraw,
			"canDeposit": canDeposit,
			"minWithdrawal": minWithdrawal,
			"maxWithdrawal": maxWithdrawal,
			"makerFee": asset["options"]["market_fee_percent"],
			"takerFee": asset["options"]["market_fee_percent"]
...
def get_trade_history2(base, quote, limit):
	...
	for result in results:
		...
		result["side1"] = get_account(result.pop("side1_account_id"))[0]["name"]
		result["side2"] = get_account(result.pop("side2_account_id"))[0]["name"]
